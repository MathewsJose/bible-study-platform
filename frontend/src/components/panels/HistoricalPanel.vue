    ...
    <div class="max-h-[calc(100vh-10rem)] overflow-y-auto px-4 py-4">
      <Transition name="content-swap" mode="out-in">
        <div :key="contentKey">
          ...
          <div v-else-if="historyStore.error && !historyStore.items.length" class="space-y-3">
            <AppError :message="historyStore.error" />
            <button
              type="button"
              class="soft-ring rounded-lg bg-rose-900 px-3 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-white"
              @click="historyStore.retry"
            >
              Retry
            </button>
          </div>
          ...
          <ul v-else class="space-y-2.5">
            ...
            >
              <div class="flex flex-col gap-2">
                <span>Could not refresh historical insight: {{ historyStore.error }}</span>
                <button
                  type="button"
...
const { selectedBook, selectedChapter, selectedVerse, hasVerseSelected } = storeToRefs(selectionStore);
const contentKey = computed(() =>
  `${selectedBook.value}:${selectedChapter.value}:${selectedVerse.value ?? 'chapter'}:${historyStore.items.length}:${historyStore.loading}`
);
</script>
