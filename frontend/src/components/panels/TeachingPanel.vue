    ...
    <div class="max-h-[calc(100vh-10rem)] overflow-y-auto px-4 py-4">
      <Transition name="content-swap" mode="out-in">
        <div :key="contentKey">
          ...
          <EmptyState
            v-else-if="!hasVerseSelected && !teachingsStore.items.length"
...
const contentKey = computed(() =>
  `${selectedBook.value}:${selectedChapter.value}:${selectedVerse.value ?? 'chapter'}:${teachingsStore.items.length}:${teachingsStore.loading}`
);
</script>
