...
final readonly class LocalProvider implements LLMProviderInterface
{
    ...
    public function complete(LLMCompletionRequest $request): LLMCompletionResponse
    {
        ...
        $baseUrl = rtrim((string) config('llm.providers.local.base_url', ''), '/');
        $this->ensureConfigured($this->identifier(), $baseUrl);
        $apiKey = (string) config('llm.providers.local.api_key', '');

        try {
            $pending = Http::timeout((int) config('llm.timeout', 30))
